...
# Resource-8: Upload index.html and error.html files
resource "aws_s3_object" "website_files" {
  ...
  bucket       = aws_s3_bucket.mywebsite.id
  key          = each.key               # "index.html" and "error.html"
  content      = file("${path.module}/${each.key}")  # Reads local files
  content_type = each.value             # Sets proper MIME type
  acl          = "public-read"          # Makes files publicly accessible

  depends_on = [
    aws_s3_bucket_policy.mywebsite
  ]
}